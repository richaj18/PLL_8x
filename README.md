...
The SOC and other ICs requires the clock signal to perform certain specific functions,and thus we need to generate
the clock signal of a particular frequency without any frequency or phase noise.<br>
...
PLL is a control system which mimics the reference signal having the same frequency and a constant phase difference 
between the reference signal and output signal generated. <br>
...
But if the output signal is leading the reference signal, thus we require another D flip flop to detect this leading 
signal called as the DOWN signal. <br>
...
But the disadvantage of this is the Dead Zone. When the phase difference between the refernce signal and output signal is very less, 
then due to the delay of gates it is not able to detect the difference and might be skipped. <br>
...
<b> Cx = C/10 </b><br>
Also, we don't want the output signal to be fluctuating, so <br>
<b> Loop BW = fref/10 </b>
